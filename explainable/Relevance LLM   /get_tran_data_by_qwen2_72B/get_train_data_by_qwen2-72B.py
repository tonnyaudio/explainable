...
def get_cot_result_thread(threadName,data_list,file_path):
    for i in tqdm( range (len(data_list))):
        input_prompt = get_cot_input(data_list[i])
        messages = get_gpt_input(cot_instruction, input_prompt)
...
if __name__ == "__main__":
    # 获取输入
    data_list = read_jsonl("./data/query_item_data.jsonl")[:]
    # 获取总相关定义
...
请你通过多个方面来生成用户query和搜索结果商品的面向用户的相关性可解释理由。
    ...
    run_num = 1
    path = f'./data/cot_result_train_thread_qwen72b/{run_num}'
    try:
        ...
        for i, file_path in enumerate(file_paths):
            ...
            _thread.start_new_thread(get_cot_result_thread, (f"Thread-{i}", sub_data_list, file_path))
            start_num = end_num
    ...
    while 1:
        pass
    output_path = f'./data/cot_result_train_{run_num}'
